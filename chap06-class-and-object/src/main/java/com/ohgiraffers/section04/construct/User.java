...
public class User {
    ...
    * 1.기본생성자(매개변수 없는 생성자)
    * 2.매개변수가 있는 생성자
    * 기본생성자는 자바 Complier가 자동으로 추가해주는 구문이지만 명시적으로 작성할 수 있다.
    * 매개변수가 있는 생성자자가 한 개라도 존재하는 경우 기본생성자를 자동으로 추가해주지 않기 때문에
    ...
    * */
    ...
    //4개의 메서드가 각각 같은이름으로 사용될 수 있도록 하는 기술을 0000이라고 합니다. 0000은 무엇인가요? 오버로딩
    ...
    //2개의 필드를 초기화 하는 생성자
    public void User(String pwd, String name){
        this.pwd = pwd;
        this.name = name;
    }
    //1개의 필드를 초기화 하는 생성자
    public void User(Date enrollDate){
        this.enrollDate = enrollDate;
    }
    */
